...
API Plane::Plane(const Vector3& normal, float distance)
{
    set(normal, distance);
}

API Plane::Plane(float normalX, float normalY, float normalZ, float distance)
{
    set(Vector3(normalX, normalY, normalZ), distance);
}

API Plane::Plane(const Plane& copy)
{
    set(copy);
}
...
API const Vector3& Plane::getNormal() const
{
    return _normal;
}

API void Plane::setNormal(const Vector3& normal)
{
    _normal = normal;
    normalize();
}

API void Plane::setNormal(float x, float y, float z)
{
    _normal.set(x, y, z);
    normalize();
}

API float Plane::getDistance() const
{
    return _distance;
}

API void Plane::setDistance(float distance)
{
    _distance = distance;
}

API float Plane::distance(const Vector3& point) const
{
    return _normal.x * point.x + _normal.y * point.y + _normal.z * point.z + _distance;
}

API void Plane::intersection(const Plane& p1, const Plane& p2, const Plane& p3, Vector3* point)
{
    _ASSERT(point);
...
API float Plane::intersects(const BoundingSphere& sphere) const
{
    return sphere.intersects(*this);
}

API float Plane::intersects(const BoundingBox& box) const
{
    return box.intersects(*this);
}

API float Plane::intersects(const Frustum& frustum) const
{
...
API float Plane::intersects(const Plane& plane) const
{
...
API float Plane::intersects(const Ray& ray) const
{
...
API bool Plane::isParallel(const Plane& plane) const
{
...
API void Plane::set(const Vector3& normal, float distance)
{
    _normal = normal;
    _distance = distance;
    normalize();
...
API void Plane::set(const Plane& plane)
{
    _normal = plane._normal;
    _distance = plane._distance;
}

API void Plane::transform(const Matrix& matrix)
{
    Matrix inverted;
